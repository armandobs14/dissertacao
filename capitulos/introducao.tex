...
Para publicar dados em formato conectado é necessário considerar etapas (ver Figura \ref{fig:modelo_processo}), ferramentas para modelagem, transformação e publicação dos dados. Além da existência de boas práticas \cite{world2014best} recomendadas para melhorar qualidade dos dados publicados. Além disso, criar links entre conjuntos de dados requer uma análise cuidadosa por parte do especialista de domínio, visto que a quantidade de novos conjuntos de dados publicados cresce constantemente.
...
\end{figure}
	  ...
	  que apesar de ser uma abordagem eficaz, não é escalável. Consequentemente, inviabilizando o processo de publicação de forma artesanal. Logo, para que seja possível constuir a Web de Dados de forma eficiente, é necessário que existam soluções capazes de conectar dados de forma automática ou semi-automática.


% Um bom exemplo de tal complexidade pode ser encontrado em alguns dos processos existentes autalmente, sendo um deles o modelo proposto por \citeonline{Avila2015}, também chamado de \textit{Piece of Cake} (PoC), que pode ser visto na Figura \ref{fig:modelo_processo}. Esse modelo possui dezenas de tarefas ao longo do processo.

%Etapas como reuso de vocabulários, conversão e enriquecimento de dados, são etapas que requerem o conhecimento de um especialista de domínio para identificar vocabulários e informações existentes na Web de forma que ,. sendo esta última responsável por conectar os dados entre bases de dados distintas, que de acordo com \citeonline{berners2006linked} é impressindível para a existência de dados conectados.
