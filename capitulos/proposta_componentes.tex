...
O componente de persistência é responsável por materializar as correspondências encontras pelo componente de alinhamento. Para isso, é utilizado o JOINT, que de acordo com \citeonline{holanda2013joint}, é um arcabouço para facilitar o desenvolvimento de aplicações baseadas em ontologia. As \textit{features} apresentadas pela ferramenta JOINT permitem que operações sejam realizada diretamente no servidor de triplas (Virtuoso, OWLim etc.). Além disso, essa ferramenta também suporta a execução de consultas SPARQL, que através de um sistema de tradução, transforma as triplas em objetos da linguagem Java.
